...
import br.com.elo7.sonda.candidato.api.error.StandardError;
...
public interface PlanetAndProbeControllerApi {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Error to register planet and probes",
                    content = @Content(mediaType = "application/json",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Probe registered on planet",
                    content = {@Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = ProbePlanetResponse.class)))}),
            @ApiResponse(responseCode = "400", description = "Error to register probe on planet",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = StandardError.class))),
            @ApiResponse(responseCode = "404", description = "Planet not found",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = StandardError.class)))})
    ResponseEntity<List<ProbePlanetResponse>> registerOnPlanet(List<@Valid ProbeRequest> probeRequests, Long planetId);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Error to move probe",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = StandardError.class))),
            @ApiResponse(responseCode = "404", description = "Probe not found",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = StandardError.class)))})
    ResponseEntity<ProbePlanetResponse> moveProbe(CommandRequest commandRequest, Long probeId);
}
